...
async function startBot() {
                ...
                message: {
                    ...
                    buttonsMessage: {
                        ...(media && typeof media === 'object' && Object.keys(media).length > 0
                            ? await generateWAMessageContent(media, { upload: chiwa.waUploadToServer })
                            : {}),
    ...
    chiwa.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        let mtype = '', mimetype = mime, convert
        ...
        try {
            ...
            if (!m)
                m = await chiwa.sendMessage(jid, { ...message, [mtype]: data }, { ...opt, ...options })
            data = null
            return m
        }
    }
    ...
    chiwa.ev.on('messages.upsert', async chatUpdate => {
        try {
            for (let messages of chatUpdate.messages) {
                ...
                messages.message = (Object.keys(messages.message)[0] === 'ephemeralMessage')
