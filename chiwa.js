...
async function startBot() {
  ...
  chiwa.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      switch (reason) {
        ...
        default:
          console.log(
            "Unknown disconnection reason, attempting to reconnect..."
          );
          startBot();
      }
    }
  ...
  chiwa.sendFile = async (
    ...
    if (/webp/.test(mime) || (/image/.test(mime) && options.asSticker))
      ...
      convert = await toAudio(data, ext);
      data = convert.data;
    ...
    let m;
    try {
      m = await chiwa.sendMessage(jid, message, { ...opt, ...options });
    } catch (e) {
      m = null;
    } finally {
      if (!m)
        m = await chiwa.sendMessage(
          jid,
          { ...message, [mtype]: data },
          { ...opt, ...options }
        );
  ...
  chiwa.getName = (jid, withoutContact = false) => {
    let id = chiwa.decodeJid(jid);
    withoutContact = chiwa.withoutContact || withoutContact;
    let v;
    if (id.endsWith("@g.us"))
      return new Promise(async (resolve) => {
        v = store.contacts[id] || {};
        if (!(v.name || v.subject)) v = (await chiwa.groupMetadata(id)) || {};
        resolve(
          v.name ||
            v.subject ||
  ...
  chiwa.sendContact = async (jid, kon, quoted = "", opts = {}) => {
    ...
    for (let i of kon) {
      list.push({
        ...
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await chiwa.getName(
          i + "@s.whatsapp.net"
        )}\nFN:${await chiwa.getName(
          i + "@s.whatsapp.net"
  ...
  chiwa.sendText = (jid, text, quoted = "", options) =>
    chiwa.sendMessage(jid, { text: text, ...options }, { quoted, ...options });

  chiwa.sendMedia = async (
    jid,
    path,
    fileName = "",
    caption = "",
  ...
  chiwa.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      for (let messages of chatUpdate.messages) {
        ...
        messages.message =
          Object.keys(messages.message)[0] === "ephemeralMessage"
            ? messages.message.ephemeralMessage.message
            : messages.message;
        if (messages.key && messages.key.remoteJid === "status@broadcast")
          return;
        if (messages.key.id.startsWith("BAE5") && messages.key.id.length === 16)
          return;
  ...
  chiwa.ev.on("group-participants.update", async (update) => {
    ...
    let welcomePath = path.join(__dirname, "plugins", "grup", "welcome.js");
    if (fs.existsSync(welcomePath)) {
      require(welcomePath)(chiwa, update);
    } else {
      console.log("Plugin welcome tidak ditemukan.");
    }
  });
}
